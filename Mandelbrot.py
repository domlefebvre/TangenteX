...
def MandelbrotDef(c,niter, horizon):  
    ...
    for n in range(niter):    # déroulement du nombre d'itérations max
        if abs(z) > horizon:  # test de divergence
            return n           # si divergence, on retourne le nb d'itérations      
...
def MandelbrotSet(xmin, xmax, ymin, ymax, Lx, Ly, niter, horizon):
    r1 = linspace(xmin, xmax, Lx)
    r2 = linspace(ymin, ymax, Ly)
    P = zeros((Lx,Ly))
    for i in range(Lx):
        for j in range(Ly):
            P[i,j] = MandelbrotDef(r1[i] + 1j*r2[j], niter, horizon)
    return P
